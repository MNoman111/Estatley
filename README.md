...
4. **Connect → Drivers** → copy the connection string, e.g.
   `mongodb+srv://USER:PASS@cluster0.xxxxx.mongodb.net/nestaro?retryWrites=true&w=majority`
   (add `/nestaro` before the `?` so it uses that database).

### 2. Push the project to GitHub
```bash
cd nestaro
git init && git add . && git commit -m "Nestaro MERN"
git branch -M main
git remote add origin https://github.com/<you>/nestaro.git
...
| Role  | Email                 |
|-------|-----------------------|
| Agent | imran@nestaro.pk  |
| Agent | ayesha@nestaro.pk |
| Agent | bilal@nestaro.pk  |
| User  | user@nestaro.pk   |
