...
const uri = mongod.getUri('zameen_clone');
...
try {
  ...
  // Login as agent
  r = await j(await fetch(`${base}/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: 'imran@zameenclone.pk', password: 'password123' }) }));
